export const numberToWordsIndian = (num) => {
  ...
  const [rupees, paise] = parseFloat(num).toFixed(2).split(".");
  const rupeeWords =
    parseInt(rupees) > 0 ? `${numToWords(parseInt(rupees))} Rupees` : "";
  const paiseWords =
    parseInt(paise) > 0 ? ` and ${numToWords(parseInt(paise))} Paise` : "";
